...
async function spinToActivity(){
  // Spin screen uses #spinWheel (older builds used #spinBtn). Guard to avoid crashes.
  const wheel = $("spinWheel") || $("spinBtn");
  if (wheel) wheel.classList.add("isSpinning");
  beep("spin");

  const a = pickActivity();
  // create a tiny sense of "working"
  await new Promise(r => setTimeout(r, 520));
  if (wheel) wheel.classList.remove("isSpinning");
